...
async def unitdata(interaction: discord.Interaction, name: str, personal: bool, classname: str, level: int,
    ...
    if unit_json is None:
        ...
        promo_class_json = None
        promo2_class_json = None
    ...
    if unit_json['growth']:
        ...
        for growth in unit_json['growth']:
            ...
            match growth:
                ...
                case _:
                    if (reeve and growth in ['luck', 'mag', 'mst']) or (salia and growth in ['str', 'skl', 'spd', 'def']):
                        bonus = 10
...
def get_class_name_choices(ctx: str) -> list[app_commands.Choice]:
    with open(trs_class_data_file, 'r') as f:
        class_json = json.load(f)
        choices = []
        for k in class_json:
            if (ctx.lower() in class_json[k]['name'].lower()) and (len(choices) < 25):
                choices.append(app_commands.Choice(name=class_json[k]['name'], value=class_json[k]['name']))
    return choices
